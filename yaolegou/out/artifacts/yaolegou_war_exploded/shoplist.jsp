...
<section>
    ...
    <div id="sp">
        ...
        <c:forEach var="sp" items="${si}">
            <%-- 这里需要修改成员和ShopInfo中的成员名一致 --%>
            <%-- 此处通过点击直接向shopdetail端口发送请求 --%>
            <dl class="nan" onclick="window.open('ShopDetail?ShopId=${sp.shopId}')" title="${sp.shopName }">
                ...
                <dd style="overflow: hidden; height: 20px;">${sp.shopMiaoShu }
                </dd>
            </dl>
        </c:forEach>
	    </div>
	    <div id="fanye">
	        <ul>
                <%-- 该部分的pb成员名需要修改的和PageBean类中的实现一致 --%>
                <li onclick="getInfo();return false;">首页</li>
                <c:forEach var="i" begin="1" end="${pb.pageCount }">
                    <c:if test="${pb.pageIndex==i }">
                        <li style="background: #ff3366;color:#fff;"
                            onclick="getInfo(${i});return false;">${i }</li>
                    </c:if>
                    <c:if test="${pb.pageIndex!=i }">
